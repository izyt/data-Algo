...
// return how many vertex is connected by this vertex
int vertexdegree(Graph g, char *vertex){
    int v = vertexID(vertex,g->vertex,g->nV);
    int degree = 0, i=0;
    for(i=0;i<g->nV;i++){
        if(g->edges[v][i]>0)degree++;
    }
    return degree;
}

// return how many vertex point to this vertex
// in degree
double vertexIn(Graph g, int v){
    double degree = 0; 
    int i=0;
    for(i=0;i<g->nV;i++){
        if(g->edges[i][v]>0)degree++;
    }
    return degree;
}
...
// return how many vertex is connected by this vertex
// out degree
double vertexOut(Graph g, int v){
    double degree = 0;
    int i=0;
    for(i=0;i<g->nV;i++){
        if(g->edges[v][i]>0)degree++;
...
// Return W-in 
// W_in = I-dest/(Sum of I of all vertex point by str)
double W_in(Graph g, char *str, char *dest){
    int s = vertexID(str,g->vertex,g->nV);
    int d = vertexID(dest,g->vertex,g->nV);
    assert(g->edges[s][d]>0);
    double sum = 0, I_d = (double)vertexIn(g, d);
    int i=0;
    for(i=0;i<g->nV;i++){
        if(g->edges[s][i]>0){
            sum += (double)vertexIn(g, i);
        }
    }
    
    if(sum == 0)return 0;
    
    return (double)I_d/sum;
}

// Return W-out
// W_out = O-dest/(Sum of O of all vertex point by str)
double W_out(Graph g, char *str, char *dest){
    
    int s = vertexID(str,g->vertex,g->nV);
    int d = vertexID(dest,g->vertex,g->nV);
    assert(g->edges[s][d]>0);
    double sum = 0, O_d = (double)vertexOut(g, d);
    int i=0;
    for(i=0;i<g->nV;i++){
        if(g->edges[s][i]>0)
            sum += (double)vertexOut(g, i);
    }
    
    if(sum == 0)return 0.5;
    
    return (double)O_d/sum;
}
