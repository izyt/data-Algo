...
void IntListInsertInOrder(IntList L, int v){
	...
	if(L->first==NULL){
	    L->first=n;
	    L->last=n;
	}else{//check where to place v
	    //insert node
	    if(v < L->first->data){//2 smallest value
	        n->next=L->first;
	        L->first=n;
	    }else if(v > L->last->data){// 3largest value
	        L->last->next=n;
	        L->last=n;
	    }else{//4 v is somewhere in the middle
	        struct IntListNode *p, *front=NULL;
	        for(p = L->first;p!=NULL;p=p->next){
	            if(v > p->data){//take done the pointer of last data smaller than v
	                front=p;
	            }
	        }
	        struct IntListNode *spare=front->next;
	        front->next=n;
	        n->next=spare;
	    }
	}
