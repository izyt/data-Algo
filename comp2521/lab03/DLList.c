...
void DLListBefore(DLList L, char *it)
{
	assert(L != NULL); 
	DLListNode *new;
	char *value = strdup(it);
	new = newDLListNode(value);
	L -> nitems ++;
	if(L->nitems == 0){//empty list
	    L -> first = new;
	    L -> curr = new;
	    L -> last = new;
	}else if (L->curr == L->first){//insert before first
	    L -> curr -> prev = new;
	    new -> next = L -> curr;
...
void DLListAfter(DLList L, char *it)
{
	...
	if(L -> nitems == 0){//empty list
	    ...
	    L -> curr = new;
	    L -> last = new;
	}else if (L->curr == L->last){//insert after last
	    L -> curr -> next = new;
	    new -> prev = L -> curr;
...
void DLListDelete(DLList L)
{
	...
	if(L -> nitems == 0){//empty list
	    //Do Nothing
	}else if(L -> nitems == 1){//only 1 element in the list
	    L -> nitems = 0;
	    free(L->first->value);
		free(L->first);
	    L -> first = NULL;
	    ...
	    p = L -> curr;
	    //change arrays
	    if(L->curr == L->first){//delete the first node
	        L->first->next -> prev = NULL;
	        L -> curr = L->curr->next;
	        L -> first = L->first -> next;
	    }else if(L->curr == L ->last){//delete the last node
...
// return number of elements in a list????? Do we need this two function
