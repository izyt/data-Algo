...
int dfsCycleCheck(Graph g, int v, int not, int visited[]){
    visited[v] = 1;
    int i;
    for(i=0;i<g->nV;i++){
        if(g->edges[v][i]>0 && i!=not){
            //printf("checking:%d %d\n",v,i);
            if(visited[i]==1){
                //printf("find:%d %d\n",v,i);
                return 1;
            }else if(dfsCycleCheck(g, i, v, visited)==1){
                //printf("find2:%d %d\n",v,i);
                return 1;
            }
        }
...
int hasCycle(Graph g) {
    ...
    for(i=0;i<g->nV;i++){
        ...
        //printf("start:%d\n",i);
        if(dfsCycleCheck(g,i,-1,visited)==1)break;
    }
    if(i==g->nV)return 0;
    else return 1;
}
...
void dfsCompo(Graph g,int v,int compID, int *compo){
    compo[v] = compID;
    int i;
    for(i=0;i<g->nV;i++){
        if(g->edges[v][i]>0 && compo[i]==-1){
            dfsCompo(g,i,compID, compo);
        }
    }
}
...
int *components(Graph g) {
    int i;
    int *compo=malloc(sizeof(int)*g->nV);
    for(i=0;i<g->nV;i++){
        compo[i]=-1;
    }
